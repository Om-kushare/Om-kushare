<h1 align="center">Hi 👋, I'm Om Kushare</h1>

<h3 align="center">Full Stack Java Developer | Java & Spring Developer</h3>

<p align="center">
  <a href="https://om-kushare.github.io/om-kushare-portfolio/" target="_blank">
  <img src="https://img.shields.io/badge/Portfolio-0A66C2?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio" />
</a>
  <a href="mailto:omkushare29@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://github.com/om-kushare" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="https://www.linkedin.com/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>



---

## 👨‍💻 About Me

* 🎓 Pursuing **M.Sc. in Computer Science** at K.K. Wagh College, Chandori.
* 💻 Passionate about **Java Backend and Full Stack Development**.
* 🌱 Currently strengthening my skills in **Java, Spring Boot, Spring MVC, MySQL, and Web Development**.
* 🚀 Experienced in developing **web applications and REST APIs**.
* 💼 Completed a **Java Full Stack Developer Internship** where I worked with Spring Boot, Spring MVC, JPA, MySQL, REST APIs, Git, and Postman.
* 🔧 Interested in building scalable and user-friendly applications.
* 📫 Reach me at **[omkushare29@gmail.com](mailto:omkushare29@gmail.com)**

---

## 🛠️ Tech Stack

### 💻 Programming Languages

<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" alt="Java" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" alt="JavaScript" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML5" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="CSS3" />
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" alt="SQL" />
</p>

### 🌱 Backend & Frameworks

<p>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white" alt="Spring Boot" />
  <img src="https://img.shields.io/badge/Spring_MVC-6DB33F?style=for-the-badge&logo=spring&logoColor=white" alt="Spring MVC" />
  <img src="https://img.shields.io/badge/Spring_JDBC-6DB33F?style=for-the-badge&logo=spring&logoColor=white" alt="Spring JDBC" />
  <img src="https://img.shields.io/badge/JPA-59666C?style=for-the-badge&logo=hibernate&logoColor=white" alt="JPA" />
  <img src="https://img.shields.io/badge/JDBC-007396?style=for-the-badge&logo=java&logoColor=white" alt="JDBC" />
</p>

### 🗄️ Databases

<p>
  <img src="https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white" alt="MySQL" />
  <img src="https://img.shields.io/badge/PL%2FSQL-F80000?style=for-the-badge&logo=oracle&logoColor=white" alt="PL/SQL" />
</p>

### 🔧 Tools & Technologies

<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" alt="Git" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  <img src="https://img.shields.io/badge/Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white" alt="Maven" />
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" alt="Postman" />
  <img src="https://img.shields.io/badge/JUnit-25A162?style=for-the-badge&logo=junit5&logoColor=white" alt="JUnit" />
  <img src="https://img.shields.io/badge/Apache_Tomcat-F8DC75?style=for-the-badge&logo=apachetomcat&logoColor=black" alt="Tomcat" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white" alt="VS Code" />
  <img src="https://img.shields.io/badge/Eclipse-2C2255?style=for-the-badge&logo=eclipseide&logoColor=white" alt="Eclipse" />
</p>

---

# 💼 Internship Experience

### Java Full Stack Developer Intern

**Creazione Software Pvt. Ltd., Pune**

📅 December 2025 – May 2026

During my internship, I:

* Developed Java web applications using **Spring Boot, Spring MVC, JPA, and MySQL**.
* Built and tested **REST APIs**.
* Implemented **CRUD operations** following MVC architecture.
* Worked on **authentication and database integration**.
* Used **Git and GitHub** for version control.
* Tested APIs using **Postman**.
* Improved my understanding of real-world software development practices.

---

# 🚚 Featured Project

## Goods Transport Network

A web-based **logistics management system** developed using Java and Spring MVC.

### 🎯 Objective

The main objective of this project is to allow customers to book vehicles according to their transportation requirements and help drivers manage delivery requests efficiently.

### 👥 Main Modules

**👨‍💼 Admin**

* Manage customers
* Manage drivers
* Manage vehicles
* Manage bookings

**🚚 Driver**

* Driver registration and login
* Vehicle registration
* View assigned transport requests
* Manage booking requests
* View booking history

**👤 Customer**

* Customer registration and login
* Book vehicles
* View booking details
* Track booking status

### 🛠️ Technologies

`Java` `Spring MVC` `Spring JDBC` `MySQL` `JSP` `HTML` `CSS` `Bootstrap` `JavaScript`

# 🏨 Hotel Management System

A responsive web-based hotel management application developed for managing hotel booking, registration, and billing operations.

### ✨ Features

* Hotel booking
* Customer registration
* Billing management
* Room availability management
* Customer record management
* Invoice generation
* Responsive user interface

### 🛠️ Technologies

`Java` `JDBC` `MySQL` `HTML` `CSS` `Bootstrap` `JavaScript`

---

# 📚 Education

### 🎓 Master of Computer Science (M.Sc.)

**K.K. Wagh Arts, Commerce, Science and Computer Science College, Chandori**

**2024 – 2026** | CGPA: **7.20**

### 🎓 Bachelor of Computer Science (B.Sc.)

**K.K. Wagh Arts, Commerce, Science and Computer Science College, Kakasaheb Nagar, Ranwad**

**2021 – 2024** | CGPA: **8.25**

---

# 📜 Certifications

* ☕ **Java Full Stack Development** — Giri's Tech Hub
* 💻 **C Programming** — Great Learning
* 🤖 **Introduction to Artificial Intelligence**
* 🧠 **LLM for Young Developers — Foundational Course**

---
I am continuously improving my **Java, Spring Boot, database, REST API, and problem-solving skills**.

---

# 🎯 Career Goal

> To start my career as a **Full Stack Java Developer**, work on real-world software applications, continuously improve my technical skills, and contribute to a professional development team.

---

# 📫 Connect With Me

<p align="center">
  <a href="mailto:omkushare29@gmail.com">
    <img src="https://img.shields.io/badge/Email-omkushare29%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://github.com/om-kushare">
    <img src="https://img.shields.io/badge/GitHub-Om%20Kushare-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
</p>

---

<p align="center">
  <b>💻 Code. Learn. Build. Improve. 🚀</b>
</p>

<p align="center">
  ⭐ Thanks for visiting my profile!
</p>
